...
def _read_signal_csv(path: Path, value_name: str) -> pd.DataFrame:
    ...
    # Try datetime-like timestamps when numeric parse fails.
    ts_dt = pd.to_datetime(ts_raw, errors="coerce")

    # Support patterns like "yyyymmddhhmmss_yyyymmddhhmmss" by taking start time.
    range_start = ts_raw.str.split("_").str[0]
    ts_range_dt = pd.to_datetime(range_start, format="%Y%m%d%H%M%S", errors="coerce")

    timestamp = ts_num.copy()
    missing = timestamp.isna()
    timestamp.loc[missing] = ts_dt.view("int64").loc[missing] / 1e9
    missing = timestamp.isna()
    timestamp.loc[missing] = ts_range_dt.view("int64").loc[missing] / 1e9
...
def build_dataset(root_dir: Path, out_dir: Path, tolerance_s: float) -> pd.DataFrame:
    ...
    for subj_dir in _iter_subject_dirs(root_dir):
        ...
        gsr_files = sorted(subj_dir.glob("*_GSR.csv"))
